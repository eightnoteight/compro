...
inline _p getit(){_p p;cin >> p;return p;}
...
int main() {
    ...
    loop (test, 1, testcases + 1) {
        vector<string> facebook = {getstr()};
        loop(len(facebook[0]) - 1)
            facebook.push_back(getstr());
        vector<vector<int32_t> > foftab(len(facebook), vector<int32_t>(len(facebook)));
        loop(x, len(facebook))
            loop(y, len(facebook))
                if (facebook[x][y] == 'Y')
                    loop(z, len(facebook))
                        if (facebook[y][z] == 'Y' && facebook[x][z] != 'Y')
                            foftab[x][z] = 1;
        vector<int32_t> counts(len(facebook));
        loop(x, len(facebook))
            loop(y, len(facebook))
                counts[x] += foftab[x][y];
        auto boss = max_element(counts.begin(), counts.end());
        print(distance(counts.begin(), boss), (*boss) - 1);
    }
    return 0;
}
