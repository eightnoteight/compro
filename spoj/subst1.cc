...
int main() {
    ...
    for (int test = 1; test <= testcases; ++test) {
        string subst1;
        cin >> subst1;
        vector<int32_t> sufarr = getSuffixArray(subst1);
        vector<int32_t> lcparr = getLongestCommonPrefixArray(subst1, sufarr);
        int32_t distinct = 0;
        for (int32_t x = 0; x < subst1.size(); ++x) {
            distinct += subst1.size() - sufarr[x] - lcparr[x];
        }
        cout << distinct << endl;
    }
    return 0;
